...
<figure>
  <img src="{{ '/assets/images/papers/lora/method-01.png' | relative_url }}" alt="LoRA reparameterization diagram showing frozen pretrained weights and a small trainable low-rank update branch with matrices A and B." />
  <figcaption>Paper figure: LoRA’s core move is to freeze the original weight matrix and learn only a low-rank update, which keeps adaptation small enough to train and store cheaply.</figcaption>
</figure>
...
<figure>
  <img src="{{ '/assets/images/papers/lora/results-08.png' | relative_url }}" alt="LoRA results page comparing GPT-3 adaptation methods, including trainable parameter counts and downstream performance." />
  <figcaption>Paper figure: the point is not just that LoRA works, but that it stays competitive while training only a tiny fraction of the parameters that full fine-tuning would require.</figcaption>
</figure>
